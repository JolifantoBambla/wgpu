...
fn map_compute_pass_timestamp_write(
    timestamp_writes: &crate::ComputePassTimestampWrites,
) -> JsValue {
    let query_set =
        &<<Context as crate::Context>::QuerySetId>::from(timestamp_writes.query_set.id).0;

    let mut mapped_timestamp_writes = web_sys::GpuComputePassTimestampWrites::new(query_set);
    mapped_timestamp_writes.beginning_of_pass_write_index(timestamp_writes.beginning_of_pass_write_index);
    mapped_timestamp_writes.end_of_pass_write_index(timestamp_writes.end_of_pass_write_index);
    mapped_timestamp_writes
}
...
impl crate::context::Context for Context {
    ...
    fn command_encoder_begin_compute_pass(
        ...
        let mapped_timestamp_writes = map_compute_pass_timestamp_write(&desc.timestamp_writes);
